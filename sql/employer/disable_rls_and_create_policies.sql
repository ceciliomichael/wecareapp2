-- Disable Row Level Security (RLS) on employers table for direct access without authentication
-- This allows the anon key to access the table directly

-- Disable RLS on employers table
...
-- CREATE POLICY "Allow anonymous read access for auth" ON employers
--   FOR SELECT USING (true);

-- Create policy to allow anonymous insert for registration
-- CREATE POLICY "Allow anonymous insert for registration" ON employers
--   FOR INSERT WITH CHECK (true);

-- Create policy to allow anonymous update (for profile updates)
-- CREATE POLICY "Allow anonymous update" ON employers
