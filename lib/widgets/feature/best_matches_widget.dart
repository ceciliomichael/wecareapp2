...
class _BestMatchesWidgetState extends State<BestMatchesWidget> {
  ...
  Future<void> _loadBestMatches() async {
    ...
    try {
      if (widget.currentHelper == null) {
        ...
        final trending = await JobPostingService.getTrendingJobPostings(limit: 15);
  ...
  Widget _buildTrendingJobsSection() {
    ...
    final uniqueTrendingJobs = _trendingJobs
        .where((job) => !_bestMatches.any((bestMatch) => bestMatch.id == job.id))
        .toList();
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        ...
                        const SizedBox(height: 12),
                        _buildTipItem(Icons.location_on, 'Update your location'),
                      ],
                    ),
                    const SizedBox(height: 24),
                    ElevatedButton.icon(
